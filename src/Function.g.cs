...
namespace GeoJSONExporter
{
    public class Function
    {
        ...
        private IModelStore<GeoJSONExporterInputs> store;

        public async Task<GeoJSONExporterOutputs> Handler(GeoJSONExporterInputs args, ILambdaContext context)
        {
            ...
            if(this.store == null)
            {
                this.store = new S3ModelStore<GeoJSONExporterInputs>(RegionEndpoint.USWest1);
            }

            var l = new ExporterInvocationWrapper<GeoJSONExporterInputs,GeoJSONExporterOutputs>(store, GeoJSONExporter.Execute);
